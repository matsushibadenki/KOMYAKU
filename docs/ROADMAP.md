...
- [Next] Add the secondary branch graph and repeat remaining keyboard/language/viewport flows on native hosts. The translated comparison controls already collapse to one column on narrow screens.
- [Done] Run a real SQLite 100k-ASCII-grapheme/1,000-Version/20-Branch fixture, plus a separate multilingual 100k-grapheme encoding/Diff/v1 Archive fixture; record timings, byte sizes and sampled memory. Confirm the current history-list cap is 500 despite 1,000 stored Versions. These are diagnostic baselines, not packaged-UI acceptance or full-history export evidence.
- [Next] Add older-Version paging, fix performance acceptance budgets, measure packaged startup/memory, and run the target-user task trial. Actual pilot users and full-history Archive functionality remain separate requirements.
