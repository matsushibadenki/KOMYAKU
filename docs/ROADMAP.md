...
Earlier consolidated verification: [2026-09-09 results and performance baseline](testing/verification-2026-09-09.md). The current History Archive package record is [2026-09-22](testing/history-archive-packaged-qa.md): 407 Bun tests pass with 25 PostgreSQL-dependent tests skipped, 31 Rust tests pass with one ignored, and 44 browser E2E tests pass. The dedicated macOS bundle builds, but its native WebView quit/relaunch gate remains open.
...
- [Next] Let the author choose or manually edit a Canonical integration result, preview its final diff and Asset closure, then persist a two-parent Version with an expected-head check. Resolve or clearly stop unsupported ambiguous histories before enabling save.
